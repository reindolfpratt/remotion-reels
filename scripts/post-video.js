...
// Secrets pulled from GitHub Environment Variables
...
async function main() {
  ...
  // 1. Fetch the next scheduled pending video
  ...
  try {
    ...
    if (IG_ACCOUNT_ID && IG_ACCESS_TOKEN) {
      ...
      const containerRes = await axios.post(`https://graph.facebook.com/v19.0/${IG_ACCOUNT_ID}/media`, {
        media_type: 'REELS',
        video_url: videoUrl,
        caption: caption,
      ...
      // Step B: Publish the container (Wait 30s for Meta to process the video file)
      ...
      await axios.post(`https://api.linkedin.com/v2/ugcPosts`, {
        ...
        specificContent: {
          "com.linkedin.ugc.ShareContent": {
            ...
            media: [{ status: "READY", title: { text: video.video_id }, originalUrl: videoUrl }]
          }
        },
    ...
    // 4. Update Supabase
    await supabase.from('content_queue').update({ status: 'published' }).eq('id', video.id);
    console.log("✅ Successfully marked video as published in Supabase Queue.");
